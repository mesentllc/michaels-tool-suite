...
public class ManifestService {
	...
	private void dumpResults(Map<String, Set<String>> results) throws IOException {
		BufferedWriter bw = new BufferedWriter(new FileWriter("/20190603_dump.txt"));
		for (String key : results.keySet()) {
...
//		service.getPackageReferences();
		...
		service.dumpResults(service.ripEfnAndPics("V:\\PostageManifest\\staging\\EVS_V2_201906030001_9275099990935650494004_mmcvz6.manifest.tmp"));
	}

	private Map<String, Set<String>> ripEfnAndPics(String filename) throws IOException {
		...
		while (br.ready()) {
			...
			if (line.startsWith("D1|")) {
				String[] parts = line.split("\\|");
				if (results.containsKey(efn)) {
					details = results.get(efn);
				}
				else {
					details = new TreeSet<>();
					results.put(efn, details);
				}
				details.add(parts[1]);
			}
		}
